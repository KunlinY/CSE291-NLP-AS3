...
import edu.berkeley.nlp.assignments.parsing.util.MetaClass;
...
public class RedwoodConfiguration  {
  ...
  public RedwoodConfiguration output(final String method) {
    if (method.equalsIgnoreCase("stdout") || method.equalsIgnoreCase("out")){
      edu.berkeley.nlp.assignments.parsing.util.logging.JavaUtilLoggingAdaptor.adapt();
  ...
  public static class Handlers {
    ...
    public static final Thunk slf4j = (config, root) -> {
      try {
        OutputHandler handler = MetaClass.create("edu.berkeley.nlp.assignments.parsing.util.logging.SLF4JHandler").createInstance();
        handler.leftMargin = config.channelWidth;
        root.addChild(handler);
      } catch (Exception e) {
