...
import edu.berkeley.nlp.assignments.parsing.international.arabic.ArabicMorphoFeatureSpecification;
...
import edu.berkeley.nlp.assignments.parsing.trees.international.arabic.*;
import edu.berkeley.nlp.assignments.parsing.trees.tregex.*;
...
public class ArabicTreebankParserParams extends AbstractTreebankParserParams  {
  ...
  public Lexicon lex(Options op, Index<String> wordIndex, Index<String> tagIndex) {
    if(op.lexOptions.uwModelTrainer == null) {
      op.lexOptions.uwModelTrainer = "edu.berkeley.nlp.assignments.parsing.parser.lexparser.ArabicUnknownWordModelTrainer";
    }
    if(morphoSpec != null) {
...
//      optionsString.append(" (removed -markFem)\n");
    ...
    //      try {
    //        WordSegmenter aSeg = (WordSegmenter) Class.forName("edu.berkeley.nlp.assignments.parsing.wordseg.ArabicSegmenter").newInstance();
    ...
    //        ex.printStackTrace();
    //      } catch (Exception e) {
    //        log.info("Couldn't instantiate segmenter: edu.berkeley.nlp.assignments.parsing.wordseg.ArabicSegmenter");
    //        e.printStackTrace();
    //      }
    //      i++; // 2 args
    //      didSomething = true;
