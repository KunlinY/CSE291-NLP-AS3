 ...
 * {@link edu.berkeley.nlp.assignments.parsing.trees.AbstractTreebankLanguagePack#getBasicCategoryFunction()}.
 ...
 * to use a {@link edu.berkeley.nlp.assignments.parsing.trees.TreeNormalizer} to remove functional
...
public abstract class TregexPattern implements Serializable  {
   ...
   *
   * {@code
   * java edu.berkeley.nlp.assignments.parsing.trees.tregex.TregexPattern [[-TCwfosnu] [-filter] [-h <node-name>]]* pattern filepath
   * }
   *
   *
   * Arguments:
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (args.length < 1) {
      errPW.println("Usage: java edu.berkeley.nlp.assignments.parsing.trees.tregex.TregexPattern [-T] [-C] [-w] [-f] [-o] [-n] [-s] [-filter]  [-hf class] [-trf class] [-h handle]* [-e ext] pattern [filepath]");
      return;
    }
