...
import edu.berkeley.nlp.assignments.parsing.semgraph.SemanticGraphFactory;
import edu.berkeley.nlp.assignments.parsing.trees.international.pennchinese.CTBErrorCorrectingTreeNormalizer;
...
public class GrammaticalStructureConversionUtils {
  ...
  private static DependencyReader loadAlternateDependencyReader(String altDepReaderName) {
    ...
    if (altDepReaderClass == null) {
      try {
        Class<?> cl = Class.forName("edu.berkeley.nlp.assignments.parsing.trees." + altDepReaderName);
        altDepReaderClass = cl.asSubclass(DependencyReader.class);
      } catch (ClassNotFoundException e) {
        //
      }
    ...
    if (altDepReaderClass == null) {
      log.info("Can't load dependency reader " + altDepReaderName + " or edu.berkeley.nlp.assignments.parsing.trees." + altDepReaderName);
      return null;
    }
  ...
  private static DependencyPrinter loadAlternateDependencyPrinter(String altDepPrinterName) {
    ...
    if (altDepPrinterClass == null) {
      try {
        Class<?> cl = Class.forName("edu.berkeley.nlp.assignments.parsing.trees." + altDepPrinterName);
        altDepPrinterClass = cl.asSubclass(DependencyPrinter.class);
      } catch (ClassNotFoundException e) {
        //
      }
  ...
  private static Function<List<? extends HasWord>, Tree> loadParser(String parserFile, String parserOptions, boolean makeCopulaHead) {
    ...
    try {
      ...
      Method method = Class.forName("edu.berkeley.nlp.assignments.parsing.parser.lexparser.LexicalizedParser").getMethod("loadModel", classes);
  ...
  public enum ConverterOptions {
    UniversalEnglish("en", new NPTmpRetainingTreeNormalizer(0, false, 1, false),
        "edu.berkeley.nlp.assignments.parsing.parser.lexparser.EnglishTreebankParserParams", false, true),
    UniversalChinese("zh", new CTBErrorCorrectingTreeNormalizer(false, false, false, false),
        "edu.berkeley.nlp.assignments.parsing.parser.lexparser.ChineseTreebankParserParams", false, false),
    English("en-sd", new NPTmpRetainingTreeNormalizer(0, false, 1, false),
        "edu.berkeley.nlp.assignments.parsing.parser.lexparser.EnglishTreebankParserParams", true, true),
    Chinese("zh-sd", new CTBErrorCorrectingTreeNormalizer(false, false, false, false),
        "edu.berkeley.nlp.assignments.parsing.parser.lexparser.ChineseTreebankParserParams", true, false);
   ...
   * edu.berkeley.nlp.assignments.parsing.parser.lexparser.EnglishTreebankParserParams,
   ...
   * <p>
   * Usage: <br>
   * <code>java edu.berkeley.nlp.assignments.parsing.trees.GrammaticalStructure [-treeFile FILE | -sentFile FILE | -conllxFile FILE | -filter] <br>
  ...
  public static void convertTrees(String[] args, String defaultLang) {
    ...
    if (test) {
      ...
      try {
        Class sgf = Class.forName("edu.berkeley.nlp.assignments.parsing.semgraph.SemanticGraphFactory");
    ...
    for (GrammaticalStructure gs : gsBank) {
      ...
      if (test) { // print the grammatical structure, the basic, collapsed and CCprocessed
        ...
        if (m != null) {
          try {
            ...
            Class sg = Class.forName("edu.berkeley.nlp.assignments.parsing.semgraph.SemanticGraph");
      ...
      if (portray) {
        try {
          ...
          Class sgu = Class.forName("edu.berkeley.nlp.assignments.parsing.rte.gui.SemanticGraphVisualization");
